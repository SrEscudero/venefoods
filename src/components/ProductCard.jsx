...
export default function ProductCard({ product, onAdd, cart = [] }) {
  ...
  return (
    <div
      className={`bg-white rounded-2xl p-5 relative group shadow-sm transition-all duration-300 border border-gray-100/80 flex flex-col h-full 
        ${isOutOfStock ? 'opacity-70 grayscale' : 'hover:shadow-xl hover:-translate-y-1 hover:border-blue-200/50'}
      `}
    >
      {/* --- ETIQUETAS FLOTANTES (BADGES) --- */}
      {isOutOfStock && (
        <span className="absolute top-4 left-4 z-20 px-3 py-1.5 rounded-full text-[10px] font-bold tracking-wider shadow-md bg-slate-800 text-white rotate-[-2deg]">
          AGOTADO
        </span>
      )}

      {!isOutOfStock && isLowStock && (
        <span className="absolute top-4 left-4 z-20 px-3 py-1.5 rounded-full text-[10px] font-bold tracking-wider shadow-md bg-red-100 text-red-600 flex items-center gap-1 animate-pulse border border-red-200">
      ...
      {!isOutOfStock && !isLowStock && product.badge && (
        <span
          className={`absolute top-4 left-4 z-20 px-3 py-1.5 rounded-full text-[10px] font-bold tracking-wider shadow-md ${getBadgeColor(
            product.badge.color
          )}`}
        >
          {product.badge.text}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <h3 className="font-bold text-slate-900 leading-tight mb-2 line-clamp-2 text-[15px]">
          {product.name}
        </h3>
        ...
        <div className="mt-auto flex items-end justify-between gap-2 border-t border-gray-100 pt-3">
            ...
            className={`w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 shadow-md
              ${
                isOutOfStock
                  ...
                  : 'bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white hover:scale-110 hover:shadow-lg shadow-blue-200/50'
              }
            `}
            title={
              isOutOfStock
          ...
          >
            {isOutOfStock ? (
              <ShoppingCart size={18} className="line-through opacity-50" />
            ) : (
